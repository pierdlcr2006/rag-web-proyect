...
export const ConversationSidebar: React.FC = () => {
  ...
  const createMutation = useMutation({
    mutationFn: () => conversationsApi.create('Nueva conversación'),
    onSuccess: (newConv) => {
          ...
          (conversations?.data || []).map((conv) => (
            <div key={conv.id} className="group relative">
              ...
              <button
                ...
                className="absolute right-4 top-1/2 -translate-y-1/2 p-1.5 text-slate-700 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all duration-200 hover:bg-red-400/10 rounded-lg"
              >
                <Trash2 size={14} />
              </button>
            </div>
